...
Releases are tagged on the `main` branch; active development happens on `dev`.
...
- **SID demo ROM: resonance and filter-mode sections** — `roms/6502/sid_demo.s` gains section 4 (fixed cutoff, resonance stepping 0→15, exercising the previously unstable high-Q range) and section 5 (fixed cutoff and resonance, cycling LP → BP → HP one scale pass each, so the BP and HP filter outputs are exercised — they had no coverage before)

### Fixed
- **SID hard sync did not reset the oscillator** — on a source wrap the sync path set the target's phase *increment* to zero, which stalled the target oscillator for one sample instead of restarting its waveform; real hard sync zeroes the target's phase accumulator. Sync now sets the phase to 0 via an explicit per-voice `syncReset` flag computed from the pre-advance phase snapshot, so all three voices see a consistent view regardless of iteration order. Verified by observing voice 3's accumulator through the `OSC3` (`$1B`) read-back register: enabling SYNC adds one guaranteed exact-zero phase reading per source wrap (measured 30 extra zeros against 27.3 predicted wraps)
...
- **GUI decomposition** — the ~110-line `drawPeripheralsMenu()` is split into `drawPeripheralEntry()` (one peripheral's row), `mountDriveImage()` (mount + warp-trap arming), and `performWarpLoad()` (the KERNAL ILOAD handler, previously an inline lambda); the C64 warp-load zero-page addresses are now named constants in a `C64ZP` namespace. The two keyboard-matrix injectors share a new `drawKeyCell()` helper for the per-cell button styling and press/release detection instead of duplicating it
- **Shared Commodore text helpers** — the duplicated `cbmMatch()` wildcard matcher and `petsciiToAscii()` converter (previously copy-pasted into both `D64Image` and `T64Image`) are consolidated into a header-only `cbm` namespace (`src/emulator/devices/CBMText.h`); both parsers now share one implementation
- **6502 core readability** — the reset/IRQ/NMI/BRK vector addresses and the stack page are now named constants (`kVecReset`/`kVecIRQ`/`kVecNMI`/`kStackBase`) instead of repeated literals, and the eight `Bxx` branch instructions now share a single `doBranch(bool)` helper rather than eight copies of the same page-cross logic (no behavioural change)
...
- **Generated ROM hygiene** — `roms/z80/hello.prg` is no longer tracked in git (it is assembled by the build and already covered by `.gitignore`, like every other generated ROM), so clean builds no longer show it as a spurious deletion

### Fixed
- **6502 decimal (BCD) mode** — `ADC`/`SBC` now honour the `D` flag and perform binary-coded-decimal arithmetic instead of always computing binary results; previously `SED`/`CLD` set the flag but it was never consulted, so any program using decimal mode produced wrong values. NMOS flag quirks are reproduced for the NMOS cores (6502/6510/8502: `Z`/`N`/`V` reflect the un-corrected intermediate), while the WDC 65C02 gets decimal-correct `Z`/`N` flags and the extra cycle real CMOS hardware takes
...
- **Dead/unused code flagged by the new warnings** — removed an unused `vec` computation in the Z80 IM 2 interrupt path (the vector is loaded via `rd16`), two never-called `readXY`/`writeXY` lambdas in the Z80 DD/FD CB handler, and unused `oldAtn`/`prevBusData` locals in `CIA6526`
...
### Added
- **Full UI state persistence** — all panel open/closed flags (`showScreen_`, `showTerminal_`, `showCpuState_`, `showDisasm_`, `showMemView_`, `showBreakpoints_`, `showWatchpoints_`, `showDesigner_`), per-device and per-peripheral panel visibility, Disassembler/Memory Viewer follow-PC toggles, ROM edit toggle, breakpoints, and watchpoints are now saved to `last_session.json` on exit and restored on startup; `imgui_layout.ini` is also moved to the OS preferences directory alongside `last_session.json` so layout is consistent across working directories
...
- **`IPeripheral` interface** (`src/emulator/core/IPeripheral.h`) — `peripheralName()`, `mountedImage()`, `mount(path)`, `eject()`, `mountError()` pure virtuals; decouples the UI from concrete peripheral types
- **`D64Image`** (`src/emulator/devices/D64Image.h/.cpp`) — CBM .d64 disk image parser: 35-track geometry, sectors-per-track table (21/19/18/17 by group), directory at track 18, linked-sector chain reads, PETSCII→ASCII conversion, `firstPRG()` / `findPRG(name)`, `diskName()`, `freeBlocks()`
- **`Drive1541`** (`src/emulator/devices/Drive1541.h/.cpp`) — MOS 1541 disk drive implemented as a software CBM Serial (IEC) state machine; inherits `IIECDevice`, `IPeripheral`, `IHasPanel`; handles LISTEN/TALK/ATN command decode (LISTEN, TALK, UNLISTEN, secondary addresses), two-phase CLK bit transfer, LOAD `"*",8,1` workflow serving the first PRG from a mounted .d64; ImGui panel shows bus line state, transfer state, and mounted image
...
- **Memory viewer ROM edit toggle** — "ROM: Read-Only" / "ROM: Editable" button in the Memory Viewer toolbar; when editable, writes to ROM-mapped addresses patch the in-memory ROM data directly; resets to read-only on every preset load and New Machine
